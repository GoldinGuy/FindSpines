    ...
    with tf.name_scope(scope, 'BooleanMask'):
        ...
        if use_static_shapes:
            ...
            for field in fields:
                ...
                subfieldlist = tf.boolean_mask(
                    boxlist.get_field(field), indicator)
...
def sample_boxes_by_jittering(boxlist,
    ...
    with tf.name_scope(scope, 'SampleBoxesByJittering'):
        ...
        rand_miny_gaussian = tf.random_normal(
            [num_boxes_to_sample], stddev=stddev)
        rand_minx_gaussian = tf.random_normal(
            [num_boxes_to_sample], stddev=stddev)
        rand_maxy_gaussian = tf.random_normal(
            [num_boxes_to_sample], stddev=stddev)
        rand_maxx_gaussian = tf.random_normal(
            [num_boxes_to_sample], stddev=stddev)
