...
class SsdMetaArchTest(ssd_meta_arch_test_lib.SSDMetaArchTestBase,
    ...
    def test_postprocess_results_are_correct(self, use_keras):
        ...
        for input_shape in input_shapes:
            ...
            self.assertAllClose(
                detections_out['detection_scores'], expected_scores)
            self.assertAllClose(
                detections_out['detection_classes'], expected_classes)
    ...
    def test_loss_results_are_correct(self, use_keras):
        ...
        batch_size = 2
        preprocessed_input = np.random.rand(
            batch_size, 2, 2, 3).astype(np.float32)
            ...
            if use_keras:
                ...
                with tf.variable_scope('mock_model'):
                    net = slim.conv2d(image, num_outputs=32, kernel_size=1,
                                      scope='layer1')
                    slim.conv2d(net, num_outputs=3,
                                kernel_size=1, scope='layer2')
