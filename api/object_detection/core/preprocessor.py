...
def _apply_with_random_selector_tuples(x,
    ...
    for case in range(num_cases):
        new_x = [control_flow_ops.switch(
            t, tf.equal(rand_sel, case))[1] for t in x]
...
def random_horizontal_flip(image,
    ...
    with tf.name_scope('RandomHorizontalFlip', values=[image, boxes]):
        ...
        if keypoints is not None and keypoint_flip_permutation is not None:
            ...
            keypoints = tf.cond(
                do_a_flip_random,
                lambda: keypoint_ops.flip_horizontal(
...
def random_vertical_flip(image,
    ...
    with tf.name_scope('RandomVerticalFlip', values=[image, boxes]):
        ...
        if keypoints is not None and keypoint_flip_permutation is not None:
            ...
            keypoints = tf.cond(
                do_a_flip_random,
                lambda: keypoint_ops.flip_vertical(
                    keypoints, 0.5, permutation),
                lambda: keypoints)
            result.append(keypoints)
    ...
    with tf.name_scope('RandomAdjustSaturation', values=[image]):
        ...
        def _adjust_saturation(image):
            image = tf.image.adjust_saturation(
                image / 255, saturation_factor) * 255
            image = tf.clip_by_value(
                image, clip_value_min=0.0, clip_value_max=255.0)
            return image
...
def _strict_random_crop_image(image,
    ...
    with tf.name_scope('RandomCropImage', values=[image, boxes]):
        ...
        if label_confidences is not None:
            new_label_confidences = overlapping_boxlist.get_field(
                'label_confidences')
            result.append(new_label_confidences)

        if multiclass_scores is not None:
            new_multiclass_scores = overlapping_boxlist.get_field(
                'multiclass_scores')
...
def random_crop_to_aspect_ratio(image,
    ...
    with tf.name_scope('RandomCropToAspectRatio', values=[image]):
        ...
        offset_height = _random_integer(
            0, orig_height - target_height + 1, seed)
        ...
        if masks is not None:
            ...
            new_masks = tf.slice(masks_inside_window,
                                 masks_box_begin, masks_box_size)
            result.append(new_masks)
...
def scale_boxes_to_pixel_coordinates(image, boxes, keypoints=None):
    ...
    if keypoints is not None:
        scaled_keypoints = keypoint_ops.scale(
...
def random_self_concat_image(
    ...
    def gen_func(): return (concat_vertical, concat_horizontal)
    params = _get_or_create_preprocess_rand_vars(
