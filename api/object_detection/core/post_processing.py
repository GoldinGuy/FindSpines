...
def multiclass_non_max_suppression(boxes,
    ...
    with tf.name_scope(scope, 'MultiClassNonMaxSuppression'):
        ...
        if clip_window is not None:
            ...
            non_zero_box_area = tf.cast(
                box_list_ops.area(sorted_boxes), tf.bool)
            ...
            sorted_boxes.add_field(
                fields.BoxListFields.scores, sorted_boxes_scores)
...
def batch_multiclass_non_max_suppression(boxes,
    ...
    with tf.name_scope(scope, 'BatchMultiClassNonMaxSuppression'):
        ...
        if clip_window.shape.ndims == 1:
            clip_window = tf.tile(tf.expand_dims(
                clip_window, 0), [batch_size, 1])
        ...
        def _single_image_nms_fn(args):
            ...
            if use_static_shapes:
                ...
                per_image_scores = tf.where(
                    tf.less(
                        tf.range(total_proposals[0]), per_image_num_valid_boxes),
                    per_image_scores,
